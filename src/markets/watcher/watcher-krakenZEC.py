#!/usr/bin/python
#
# Coinorama/coinref: watch and store raw Kraken ZEC market info
#
...
# Watcher class
class KrakenZECWatcher (coinwatcher.CoinWatcher) :
    def __init__ ( self, shortname, with_coinrefd, logger ):
        ...
        self.ZEC_USD_rate = 3.0
        self.ZEC_USD_stamp = 0
    ...
    def buildData ( self, book, trades, lag ):
        ...
        try:
            for t in trades['result']['XZECXXBT']:
                tprice = float ( t[0] )
                tvol = tprice * float ( t[1] ) # amount is ZEC, convert to BTC using price
                tdate = float ( t[2] )
                ...
                if ( tdate > mostRecent ):
                    mostRecent = tdate
                    mostRecentPrice = 1 / tprice  # amount is ZEC, convert to BTC using price
        except Exception:
            self.logger.write ( 'error buildData trades\n' + str(traceback.format_exc()) )
            return None

        try: # bids and asks are inverted
            for b in book['result']['XZECXXBT']['asks']:
            ...
            for a in book['result']['XZECXXBT']['bids']:
        ...
        try:
            ...
            ed.USD_conv_rate = self.ZEC_USD_rate
        except Exception:
    ...
    def fetchData ( self ):
        if ( (time.time()-self.ZEC_USD_stamp) > 60.0 ): # get USD/ZEC rate every minute
            try:
