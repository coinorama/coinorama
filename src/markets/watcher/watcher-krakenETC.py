#!/usr/bin/python
#
# Coinorama/coinref: watch and store raw Kraken ETC market info
#
...
# Watcher class
class KrakenETCWatcher (coinwatcher.CoinWatcher) :
    def __init__ ( self, shortname, with_coinrefd, logger ):
        ...
        self.ETC_USD_rate = 3.0
        self.ETC_USD_stamp = 0
    ...
    def buildData ( self, book, trades, lag ):
        ...
        try:
            for t in trades['result']['XETCXXBT']:
                tprice = float ( t[0] )
                tvol = tprice * float ( t[1] ) # amount is ETC, convert to BTC using price
                tdate = float ( t[2] )
                ...
                if ( tdate > mostRecent ):
                    mostRecent = tdate
                    mostRecentPrice = 1 / tprice  # amount is ETC, convert to BTC using price
        except Exception:
            self.logger.write ( 'error buildData trades\n' + str(traceback.format_exc()) )
            return None

        try: # bids and asks are inverted
            for b in book['result']['XETCXXBT']['asks']:
            ...
            for a in book['result']['XETCXXBT']['bids']:
        ...
        try:
            ...
            ed.USD_conv_rate = self.ETC_USD_rate
        except Exception:
    ...
    def fetchData ( self ):
        if ( (time.time()-self.ETC_USD_stamp) > 60.0 ): # get USD/ETC rate every minute
            try:
