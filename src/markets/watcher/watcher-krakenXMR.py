#!/usr/bin/python
#
# Coinorama/coinref: watch and store raw Kraken XMR market info
#
...
# Watcher class
class KrakenXMRWatcher (coinwatcher.CoinWatcher) :
    def __init__ ( self, shortname, with_coinrefd, logger ):
        ...
        self.XMR_USD_rate = 132.5
        self.XMR_USD_stamp = 0
    ...
    def buildData ( self, book, trades, lag ):
        ...
        try:
            for t in trades['result']['XXMRXXBT']:
                tprice = float ( t[0] )
                tvol = tprice * float ( t[1] ) # amount is XMR, convert to BTC using price
                tdate = float ( t[2] )
                ...
                if ( tdate > mostRecent ):
                    mostRecent = tdate
                    mostRecentPrice = 1 / tprice  # amount is XMR, convert to BTC using price
        except Exception:
            self.logger.write ( 'error buildData trades\n' + str(traceback.format_exc()) )
            return None

        try: # bids and asks are inverted
            for b in book['result']['XXMRXXBT']['asks']:
            ...
            for a in book['result']['XXMRXXBT']['bids']:
        ...
        try:
            ...
            ed.USD_conv_rate = self.XMR_USD_rate
        except Exception:
    ...
    def fetchData ( self ):
        if ( (time.time()-self.XMR_USD_stamp) > 60.0 ): # get USD/XMR rate every minute
            try:
                connection = httplib.HTTPSConnection ( 'api.kraken.com', timeout=5 )
                connection.request ( 'GET', '/0/public/Ticker?pair=XXMRZUSD' )
        ...
        ed = coinwatcher.CoinWatcher.fetchData ( self, httplib.HTTPSConnection, 'api.kraken.com', '/0/public/Depth?pair=XXMRXXBT', trades )
        return ed
