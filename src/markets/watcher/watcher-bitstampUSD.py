#!/usr/bin/python
#
# Coinorama/coinref: watch and store raw Bitstamp USD market info
#
...
# Watcher class
class BitstampUSDWatcher (coinwatcher.CoinWatcher) :
    ...
    def fetchData ( self ):
        trades = '/api/transactions/?timedelta=%d' %  int ( (2+time.time()) - self.mostRecentTransaction )
        ed = coinwatcher.CoinWatcher.fetchData ( self, httplib.HTTPSConnection, 'www.bitstamp.net', '/api/order_book/', trades )
        return ed
...
if __name__ == "__main__":
    coinwatcher.main ( 'Bitstamp-USD', 'bitstampUSD', BitstampUSDWatcher )
