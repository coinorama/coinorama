#!/usr/bin/python
#
# Coinorama/coinref: watch and store raw Kraken CAD market info
#
...
# Watcher class
class KrakenCADWatcher (coinwatcher.CoinWatcher) :
    ...
    def buildData ( self, book, trades, lag ):
        ...
        try:
            for t in trades['result']['XXBTZCAD']:
                tvol = float ( t[1] )
                tdate = float ( t[2] )
        ...
        try:
            for b in book['result']['XXBTZCAD']['bids']:
                bprice = float ( b[0] )
                bvol = float ( b[1] )
            ...
            for a in book['result']['XXBTZCAD']['asks']:
                aprice = float ( a[0] )
                avol = float ( a[1] )
    ...
    def fetchData ( self ):
        ...
        ed = coinwatcher.CoinWatcher.fetchData ( self, httplib.HTTPSConnection, 'api.kraken.com', '/0/public/Depth?pair=XXBTZCAD', trades )
        return ed
