#!/usr/bin/python
#
# Coinorama/coinref: watch and store raw Kraken BCH market info
#
...
# Watcher class
class KrakenBCHWatcher (coinwatcher.CoinWatcher) :
    def __init__ ( self, shortname, with_coinrefd, logger ):
        ...
        self.BCH_USD_rate = 155.0
        self.BCH_USD_stamp = 0
    ...
    def buildData ( self, book, trades, lag ):
        ...
        try:
            for t in trades['result']['BCHXBT']:
                tprice = float ( t[0] )
                tvol = tprice * float ( t[1] ) # amount is BCH, convert to BTC using price
                tdate = float ( t[2] )
                ...
                if ( tdate > mostRecent ):
                    mostRecent = tdate
                    mostRecentPrice = 1 / tprice  # amount is BCH, convert to BTC using price
        except Exception:
            self.logger.write ( 'error buildData trades\n' + str(traceback.format_exc()) )
            return None

        try: # bids and asks are inverted
            for b in book['result']['BCHXBT']['asks']:
            ...
            for a in book['result']['BCHXBT']['bids']:
        ...
        try:
            ...
            ed.USD_conv_rate = self.BCH_USD_rate
        except Exception:
    ...
    def fetchData ( self ):
        if ( (time.time()-self.BCH_USD_stamp) > 60.0 ): # get USD/BCH rate every minute
            try:
                connection = httplib.HTTPSConnection ( 'api.kraken.com', timeout=5 )
                connection.request ( 'GET', '/0/public/Ticker?pair=BCHUSD' )
                r = connection.getresponse ( )
                if ( r.status == 200 ):
                    ticker_txt = r.read ( )
                    ticker_json = json.loads ( ticker_txt )
                    rate = float ( ticker_json['result']['BCHUSD']['c'][0] )
                    if ( rate > 0 ):
                        self.BCH_USD_rate = rate
                        self.BCH_USD_stamp = time.time ( )
                        #self.logger.write ( 'rate: %f ' % self.BCH_USD_rate )
                else:
                    self.logger.write ( 'error BCH_USD http %d' % r.status )
                connection.close ( )
            except Exception:
                self.logger.write ( 'error BCH_USD\n' + str(traceback.format_exc()) )
                pass

        trades = '/0/public/Trades?pair=BCHXBT&since=%s' % self.mostRecentTransactionID
        ed = coinwatcher.CoinWatcher.fetchData ( self, httplib.HTTPSConnection, 'api.kraken.com', '/0/public/Depth?pair=BCHXBT', trades )
        return ed
...
if __name__ == "__main__":
    coinwatcher.main ( 'Kraken-BCH', 'krakenBCH', KrakenBCHWatcher )
