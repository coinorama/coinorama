#!/usr/bin/python
#
# Coinorama/coinref: watch and store raw BTCTurk TRY market info
#
...
# Watcher class
class BtcturkTRYWatcher (coinwatcher.CoinWatcher):
    def __init__ ( self, shortname, with_coinrefd, logger ):
        coinwatcher.CoinWatcher.__init__ ( self, shortname, with_coinrefd, logger, delay=12 )
        self.mostRecentTransactionID = 0
        self.mostRecentPrice = 0
        self.TRY_USD_rate = 0.3788
        self.TRY_USD_stamp = 0
    ...
    def buildData ( self, book, trades, lag ):
        ...
        try:
            ...
            self.logger.write ( 'error buildDatabook %s' % e )
            return None
        ...
        try:
            ...
            ed.USD_conv_rate = self.TRY_USD_rate
        except Exception as e:
            self.logger.write ( 'error buildData %s' % e )
            return None
    ...
    def fetchData ( self ):
        # get USD/TRY rate every hour
        if ( (time.time()-self.TRY_USD_stamp) > 3600.0 ):
            try:
                connection = httplib.HTTPConnection ( 'download.finance.yahoo.com', timeout=4 )
                connection.request ( 'GET', '/d/quotes.csv?s=TRYUSD=X&f=sl1&e=.csv' )
        ...
        ed = coinwatcher.CoinWatcher.fetchData ( self, httplib.HTTPSConnection, 'www.btcturk.com', '/api/orderbook', trades )
        return ed
