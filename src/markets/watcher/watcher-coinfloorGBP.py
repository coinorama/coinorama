#!/usr/bin/python
#
# Coinorama/coinref: watch and store raw Coinfloor GBP market info
#
...
# WARNING: YOU NEED AN API KEY TO FETCH DATA
# UPDATE THE fetchDat() FUNCTION ACCORDINGLY
...
# Watcher class
class CoinfloorGBPWatcher (coinwatcher.CoinWatcher) :
    def __init__ ( self, shortname, with_coinrefd, logger ):
        ...
        self.GBP_USD_rate = 1.66090
        self.GBP_USD_stamp = 0
    ...
    def fetchData ( self ):
        if ( (time.time()-self.GBP_USD_stamp) > 3600.0 ): # get USD/GBP rate every hour
            try:
        ...
        key = '' # FILL IN YOUR API KEY HERE
        trades = '/'+key+'/XBT/GBP/trades'
        if ( self.mostRecentTransactionID != 0 ):
            trades = trades + ('?since=%s' % self.mostRecentTransactionID)
        ed = coinwatcher.CoinWatcher.fetchData ( self, httplib.HTTPSConnection, 'webapi.coinfloor.co.uk:8802', '/'+key+'/XBT/GBP/orderbook', trades )
        return ed
...
if __name__ == "__main__":
    coinwatcher.main ( 'Coinfloor-GBP', 'coinfloorGBP', CoinfloorGBPWatcher )
