(function($) {
    ...
    function init(plot) {
        function findBelowSeries( series, allseries ) {
            ...
            if ( series.fillBelowTo === 'zero' )
            {
                var xaxis = jQuery.extend ( true, {}, series );
                for ( j=xaxis.datapoints.pointsize-1; j<xaxis.datapoints.points.length; j+=xaxis.datapoints.pointsize )
                    xaxis.datapoints.points[j] = series.yaxis.min;
                return xaxis;
            }
            else
            {
